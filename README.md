...
| :--- | :--- | :--- |
| idle | 회색 | 대기 중 |
| triggering | 노란색 (깜빡임) | 워크플로우 트리거 중 |
| running | 파란색 (깜빡임) | 에이전트 실행 중 |
| complete | 초록색 | 완료 |
| failed | 빨간색 | 실패 |
...
* **Claude Code CLI**: CI 환경에서 `claude -p` 명령으로 에이전트 프롬프트를 실행. 로컬과 달리 `Task` 서브에이전트 스폰이 불가하므로, 각 에이전트의 관점을 순차적으로 수행.
* **동시성 제어**: `모드!`와 `개선!` 각각 별도 큐(`agent-create`, `agent-improve`)로 관리. 동일 큐 내 직렬 실행.
