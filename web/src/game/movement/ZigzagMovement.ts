import type { CardSprite } from '../objects/CardSprite';
import type { CardMovementData } from './CardMovementData';
import { CardMovementImpl } from './CardMovementImpl';

/** Aggressive zigzag: 7 sharp cuts with rotation snaps, like dodging bullets. */
export class ZigzagMovement extends CardMovementImpl {
  execute(duration: number): Promise<void> {
    return new Promise<void>(resolve => {
      this.resolvePromise = resolve;
      const n = this.sprites.length;
      const staggerDelay = 50;
      const perCardExtra = 70;
      const zigCount = 7;

      for (let i = 0; i < n; i++) {
        const sprite = this.sprites[i];
        const sx = this.data.startPositions[i].x;
        const sy = this.data.startPositions[i].y;
        const ex = this.data.endPositions[i].x;
        const ey = this.data.endPositions[i].y;
        const cardDuration = duration + i * perCardExtra;
        const delay = i * staggerDelay;

        const dx = ex - sx;
        const dy = ey - sy;
        const dist = Math.sqrt(dx * dx + dy * dy);
        const perpX = dist > 0 ? -dy / dist : 0;
        const perpY = dist > 0 ? dx / dist : 1;
        const amplitude = Math.max(50, dist * 0.35);

        const tw = this.scene.tweens.addCounter({
          from: 0,
          to: 1,
          duration: cardDuration,
          delay,
          ease: 'Cubic.easeOut',
          onUpdate: (tween) => {
            const t = tween.getValue() ?? 0;
            const baseX = sx + dx * t;
            const baseY = sy + dy * t;
            // Sharp triangle wave
            const phase = (t * zigCount) % 1;
            const triangle = phase < 0.5 ? (phase * 4 - 1) : (3 - phase * 4);
            // Strong dampen curve
            const dampen = Math.pow(1 - t, 1.5);
            const offset = triangle * amplitude * dampen;
            sprite.x = baseX + perpX * offset;
            sprite.y = baseY + perpY * offset;
            // Snap rotation at each direction change
            sprite.setRotation(triangle * 0.45 * dampen);
            // Brief scale pop at each zig apex
            const apexProximity = 1 - Math.abs(triangle);
            sprite.setScale(1 + apexProximity * 0.2 * dampen);
          },
          onComplete: () => {
            sprite.x = ex;
            sprite.y = ey;
            sprite.setRotation(0);
            sprite.setScale(1);
            if (i === n - 1 && this.resolvePromise) {
              this.resolvePromise();
              this.resolvePromise = null;
            }
          },
        });
        this.tweens.push(tw);
      }
    });
  }
}
